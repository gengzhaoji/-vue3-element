 ...
 */
  config.cancelToken = new axios.CancelToken(cancel => {
    // 判断是否重复切不在白名单中
    const repeat = axiosPromiseArr.some(item => item.url === config.url && item.method === config.method && isEqual(item.data, config.data));
    if (repeat && !whiteList.includes(config.url)) {
      cancel();
    } else {
      axiosPromiseArr.push({ url: config.url, method: config.method, data: config.data, cancel })
    }
  })
...
service.interceptors.response.use(res => {
  ...
  axiosPromiseArr.forEach((item, index) => {
    if (item.url === res.config.url.replace(res.config.baseURL, '') && item.method === res.config.method && isEqual(item.data, [undefined].includes(res.config.data) ? res.config.data : JSON.parse(res.config.data))) {
...
export function download(url, params, filename, currency = true) {
  ...
  return service.post(url, params, headers).then((data) => {
    ...
    if ('download' in document.createElement('a')) {
      ...
      elink.download = currency ? `${date(new Date())}-${filename}` : filename
      ...
      navigator.msSaveBlob(blob, currency ? `${date(new Date())}-${filename}` : filename)
    }
    ElMessage({
      message: '导出成功！',
      type: 'success'
