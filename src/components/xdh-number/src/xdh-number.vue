<template>
    <div :class="{ 'my-number': true, 'is-pointer': $attrs.click }" v-bind="$attrs">
        ...
        <span class="my-number__value" :class="{ [`is-${this.type}`]: !!type }" ref="container" :key="key">
...
export default {
    ...
    props: {
        ...
        type: {
            ...
            default: '',
            validator(val) {
                return ['', 'primary', 'success', 'warning', 'danger', 'info', 'default', 'default1'].includes(val);
            },
        },
        // 前缀 和 后缀采用下标显示
        sup: Boolean,
    ...
    methods: {
        ...
        getPercent(val) {
            return this.percentage ? `${(Number.parseFloat(val) * 100).toFixed(this.precision)}%` : val;
        },
        /**
         * 数字显示分隔符
         * @param val
